...
import com.example.backend_service.auth.repository.StudentEmailVerificationRepository;
...
/**
 * Verifies a student's email during signup, before any Student account exists. Mirrors
...
public class StudentEmailVerificationService {
    ...
    private static final Logger log = LoggerFactory.getLogger(StudentEmailVerificationService.class);
    ...
    public StudentEmailVerificationService(
    ...
    public void sendCode(String rawEmail) {
        ...
        StudentEmailVerification verification = verificationRepository.findById(email)
                .orElseGet(() -> new StudentEmailVerification(email));
        ...
        emailService.sendHtml(email, "Verify your email for UniVerse", buildCodeEmailHtml(code));
        log.info("Signup email verification code sent to email={}", email);
    }

    public String verifyCode(String rawEmail, String code) {
        String email = normalize(rawEmail);
        StudentEmailVerification verification = verificationRepository.findById(email).orElse(null);
    ...
    /**
     * Called from student registration to redeem the token obtained via {@link #verifyCode}.
     */
    ...
    private String buildCodeEmailHtml(String code) {
        return """
                <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; color: #1f2937;">
                    ...
                    <p>Use the code below to verify this email address and continue creating your UniVerse account. This code expires in %d minutes.</p>
